...
class BLEServiceInstance {
  ...
  destroyBLEInstance = () => this.manager.destroy()
  ...
  disconnectDevice = () => {
    if (!this.device) {
      this.showError(deviceNotConnectedErrorText)
      throw new Error(deviceNotConnectedErrorText)
    }
    return this.manager
      .cancelDeviceConnection(this.device.id)
      .then(() => this.showSuccess('Device disconnected'))
      .catch(error => {
  ...
  onBluetoothPowerOff = () => {
    this.showError('Bluetooth is turned off')
  }
  ...
  scanDevices = async (
    ...
    this.manager.startDeviceScan(UUIDs, options, (error, device) => {
      if (error) {
        this.onError(error)
  ...
  connectToDevice = (deviceId: DeviceId, connectionOptions?: ConnectionOptions) =>
  ...
  onError = (error: BleError) => {
    switch (error.errorCode) {
      ...
      case BleErrorCode.LocationServicesDisabled:
        this.showError('Location services are disabled')
        break
      default:
        this.showError(JSON.stringify(error, null, 4))
    }
  }
  ...
  requestBluetoothPermission = async () => {
    ...
    this.showError('Permission have not been granted')
